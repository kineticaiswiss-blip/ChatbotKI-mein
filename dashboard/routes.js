...
function layout(req, content){
  return `<!doctype html><html><head><meta charset="utf-8">${baseStyle(req.user?.darkMode)}</head>
<body>
<nav class="container">
...
router.post("/login",(req,res)=>{
  ...
  const acc=loadAccounts().find(a=>a.email===identifier||a.phone===identifier);
  if(!acc||!verifyPassword(password,acc.salt,acc.hash)) return res.send("❌ Login fehlgeschlagen");
...
router.get("/dashboard/account",requireAuth,(req,res)=>{
  res.send(layout(req,`<h2>Account</h2><p>${req.user.firstName} (${req.user.role})</p>`));
});
